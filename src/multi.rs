    ...
    pub(crate) fn draw(
        ...
        for &index in &self.ordering {
            ...
            let line_count = member
                .draw_state
                .as_ref()
                .map(|d| real_line_count(&d.lines, width))
                .unwrap_or_default();
            // Track the total number of zombie lines on the screen.
        ...
        let orphan_lines_count = real_line_count(&self.orphan_lines, width);
        ...
        if let Some(extra_lines) = &extra_lines {
            ...
            draw_state.orphan_lines_count += real_line_count(extra_lines, width);
        }
